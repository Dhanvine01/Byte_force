...
PII_PATTERNS = [
    ...
    ("pan","PAN Number","critical",0.98,[_r(r"\b[A-Z]{5}[0-9]{4}[A-Z]\b")]),
...
_ADDR_START = re.compile(
    r"^(c/o|s/o|d/o|w/o|address|addr|residence)$",
    re.IGNORECASE
)
# These are content words that confirm we're inside an address block
# (only used AFTER an anchor has been found)
_ADDR_CONTENT = re.compile(
    ...
    r"taluk|mandal|tehsil|vtc|village|town|po|post)",
    re.IGNORECASE
)
_ADDR_END = re.compile(
    r"^(mobile|phone|aadhaar|vid|enrollment|enrolment|email|"
    r"signature|document|download|information|note|"
    r"\d{4}\s?\d{4}\s?\d{4})$",
    re.IGNORECASE
)
...
def detect_address_blocks(page):
    ...
    anchor (Address:, C/O, S/O, etc.) and then collecting the VALUE words that
    follow it.  Only the value portion is masked — the label itself is excluded
    from the bounding box.  Returns ONE detection per address anchor with a
    tight bbox covering all value words.
    """
    words = page.get("words", [])
    page_num = page.get("page", 1)
    if not words:
    ...
    for i, w in enumerate(words):
        ...
        # ── Build ONE tight bbox for the entire address block ──────────
        ...
        # Compute per-line x-ranges, then build a bbox that is only as
        # wide as the widest line (not from global min-x to global max-x+w
        # which would span across columns).
        line_ranges = []
        ...
        # Use the x-start of the first line and width of the widest line
