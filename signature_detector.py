...
# Tunable thresholds
# ---------------------------------------------------------------------------
MIN_CONTOUR_AREA   = 3500          # Minimum pixel area to consider
MAX_CONTOUR_AREA   = 80_000      # Too large = printed text block, not sig
MIN_ASPECT_RATIO   = 2.5          # Width / Height – sigs are wide
MAX_ASPECT_RATIO   = 12.0
MIN_FILL_RATIO     = 0.05         # % of bbox filled with dark pixels
MAX_FILL_RATIO     = 0.35         # High fill → printed text block
MIN_CC_COUNT       = 6            # Connected components in ROI (cursive loops)
SIG_CONFIDENCE_THRESHOLD = 0.80   # Minimum score to label as signature
...
def detect_signatures(
    ...
    for (x, y, w, h, contour) in candidates:
        # Skip candidates in the top 15% of the image (logos, headers)
        if y < img_h * 0.15:
            continue
